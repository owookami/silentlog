...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (PermissionManager.DEV_MODE || AppDebugSettings.DEBUG_MODE_ENABLED) {
            ...
            Log.d(TAG, "앱 시작: 디버깅 모드 활성화")
        }
    }

    override fun onStop() {
        super.onStop()
        if ((PermissionManager.DEV_MODE || AppDebugSettings.DEBUG_MODE_ENABLED) && isFinishing) {
            // 앱이 완전히 종료될 때 권한 상태 저장
            PermissionManager.savePermissionResetTime(this)
            PermissionManager.logPermissionStatus(this)
            Log.d(TAG, "앱 종료 시 상태 기록 완료 (디버깅 모드)")
        }
    }
